...
# 统计信息
print(f"映射表总字段数: {len(FIELD_EN_MAP)}")
print(f"中文字段名平均长度: {sum(len(k) for k in FIELD_EN_MAP.keys()) / len(FIELD_EN_MAP):.1f}")
print(f"英文字段名平均长度: {sum(len(v) for v in FIELD_EN_MAP.values()) / len(FIELD_EN_MAP):.1f}")

# 导出为JSON格式
if __name__ == "__main__":
    import json
    with open('field_mapping.json', 'w', encoding='utf-8') as f:
        json.dump(FIELD_EN_MAP, f, ensure_ascii=False, indent=2)
    print("已导出到 field_mapping.json")
