...
FIELD_MAPPINGS = {
    ...
    "live_area_paid_leads_total": ["直播车云店+区域付费线索量"],
    "live_area_paid_leads_t": ["T月直播车云店+区域付费线索量"],
    "live_area_paid_leads_t_minus_1": ["T-1月直播车云店+区域付费线索量"],
    ...
    "avg_daily_effective_live_hours_25min_t_minus_1": [
        "T-1月日均有效（25min以上）时长（h）"
    ],
    ...
    "avg_small_wheel_clicks_per_session_t_minus_1": [
        "T-1月场均小风车点击次数"
    ],
...
def field_match(src: str, col: str) -> bool:
    ...
    logger.debug(
        f"field_match: src='{src}', col='{col}', src_parts={src_parts}, col_parts={col_parts}"
    )
...
def rename_columns_loose(
    pl_df: pl.DataFrame, mapping: Dict[str, str]
) -> pl.DataFrame:
    col_map = {}
    for src, dst in mapping.items():
        for c in pl_df.columns:
            ...
            logger.debug(
                f"字段映射尝试: 源字段='{src}'，目标字段='{c}'，match={match}"
            )
            if match:
                col_map[c] = dst
                break
...
def normalize_nsc_col(
    df: pl.DataFrame, colname: str = "NSC_CODE"
) -> pl.DataFrame:
    ...
    multi_df = (
        df.filter(mask)
        .with_columns(pl.col(colname).str.split(",").alias("_nsc_list"))
        .explode("_nsc_list")
    )
    single_df = df.filter(~mask).with_columns(
        pl.col(colname).alias("_nsc_list")
    )
    ...
    logger.debug(
        f"[NSC_CODE过滤] 原始行数: {original_count}, 过滤后: {filtered_count}"
    )
    if filtered_count == 0:
        logger.warning(
            f"[NSC_CODE过滤] 过滤后无数据，保留部分原始数据: {df.head()}"
        )
    return df


def ensure_date_column(
    pl_df: pl.DataFrame, colname: str = "date"
) -> pl.DataFrame:
    ...
    pl_df = pl_df.with_columns(
        pl.coalesce(parser_expressions).cast(pl.Date).alias(colname)
    )
...
def format_extracted_fields(
    pl_df: pl.DataFrame, file_type: str
) -> pl.DataFrame:
    """在提取字段时立即进行格式化"""
    ...
    format_rules = {
        ...
        "leads": {"small_wheel_leads": {"decimals": 0}},
        "spending": {"spending_net": {"decimals": 2}},
    }
...
def process_single_table(df, mapping, sum_cols=None, file_type=None):
    ...
    logger.debug(
        f"[process_single_table] 标准化 NSC_CODE 后列名: {after_norm_cols}"
    )
    if "NSC_CODE" not in df.columns:
        logger.error(
            "[process_single_table] normalize_nsc_col 之后 NSC_CODE 列丢失。"
        )
        raise ValueError(
            "[process_single_table] normalize_nsc_col 之后 NSC_CODE 列丢失。"
        )
    non_null_cnt = df.filter(
        pl.col("NSC_CODE").is_not_null() & (pl.col("NSC_CODE") != "")
    ).height
    if non_null_cnt == 0:
        logger.error(
            f"[process_single_table] NSC_CODE 标准化后为空。非空行计数={non_null_cnt}"
        )
        raise ValueError(
            f"[process_single_table] NSC_CODE 标准化后为空。非空行计数={non_null_cnt}"
        )
    ...
    if "date" not in df.columns:
        logger.error(
            "[process_single_table] ensure_date_column 之后未发现 date 列。"
        )
        raise ValueError(
            "[process_single_table] ensure_date_column 之后未发现 date 列。"
        )
    ...
    agg_exprs = (
        [pl.col(c).sum().alias(c) for c in sum_cols] if sum_cols else []
    )
    if agg_exprs:
    ...
    logger.debug(
        f"[process_single_table] 完成。输出列名: {list(df.columns)}, 行数={df.height}"
    )
    return df
...
def process_dr_table(df):
    ...
    if "leads_type" in df.columns:
        logger.debug(
            f"[DR PROBE] Unique 'leads_type' values: {df.get_column('leads_type').unique().to_list()}"
        )
    if "mkt_second_channel_name" in df.columns:
        logger.debug(
            f"[DR PROBE] Unique 'mkt_second_channel_name' values: {df.get_column('mkt_second_channel_name').unique().to_list()}"
        )

    dr_all = df.group_by("NSC_CODE", "date").agg(
        pl.col("leads_type")
        .filter(pl.col("leads_type") == "自然")
        .count()
        .alias("natural_leads"),
        pl.col("leads_type")
        .filter(pl.col("leads_type") == "广告")
        .count()
        .alias("ad_leads"),
        pl.col("leads_type")
        .filter(
            ...
            & (
                pl.col("mkt_second_channel_name").is_in(
                    ["抖音车云店_BMW_本市_LS直发", "抖音车云店_LS直发"]
                )
            )
        )
        .count()
        ...
        .filter(
            (pl.col("leads_type") == "广告")
            & (
                pl.col("mkt_second_channel_name")
                == "抖音车云店_BMW_总部BDT_LS直发"
            )
        )
        .count()
        .alias("area_leads"),
        pl.col("send2dealer_id")
        .filter(pl.col("send2dealer_id").cast(pl.Utf8) == pl.col("NSC_CODE"))
        .count()
        .alias("local_leads"),
    )
...
def process_account_base(all_sheets):
    ...
    for sheetname, df_sheet in sheets_data:
        ...
        if (
            hasattr(df_sheet, "dtypes")
            and str(type(df_sheet)).find("pandas") != -1
        ):
            # pandas DataFrame
        ...
        # 获取列名用于调试
        # list(df_sheet.columns) if hasattr(df_sheet, "columns") else []

        # Use loose matching for column names
        level_df_sheet = rename_columns_loose(
            df_sheet, {"NSC_id": "NSC_CODE", "第二期层级": "level"}
        )
        if (
            "NSC_CODE" in level_df_sheet.columns
            and "level" in level_df_sheet.columns
        ):
            logger.info(f"[ACC_BASE] Identified level sheet: {sheetname}")
            level_df = level_df_sheet.select(["NSC_CODE", "level"])

        store_name_df_sheet = rename_columns_loose(
            df_sheet, {"NSC Code": "NSC_CODE", "抖音id": "store_name"}
        )
        if (
            "NSC_CODE" in store_name_df_sheet.columns
            and "store_name" in store_name_df_sheet.columns
        ):
            logger.info(f"[ACC_BASE] Identified store_name sheet: {sheetname}")
            store_name_df = store_name_df_sheet.select(
                ["NSC_CODE", "store_name"]
            )

    if level_df is None and store_name_df is None:
        logger.warning(
            "[ACC_BASE] Could not find valid level or store_name sheets in account_base_file."
        )
        return None
    ...
    logger.debug(
        f"[ACC_BASE PROBE] Final merged account_base data:\n{merged.head() if merged is not None else 'None'}"
    )
    return merged
...
def process_all_files(
    ...
    if "video_excel_file" in local_paths:
        ...
        if df is not None:
            ...
            dfs["video"] = process_single_table(
                df,
                VIDEO_MAP,
                [
                    "anchor_exposure",
    ...
    if "msg_excel_file" in local_paths:
        ...
        for sheetname, df in sheets_data:
            ...
            try:
                ...
                logger.error(
                    f"[MSG日期] sheet标题 '{date_str}' 不是有效日期格式: {e}"
                )
            ...
            logger.debug(
                f"[MSG调试] sheet={sheetname}, polars columns={df.columns}"
            )
            df = normalize_nsc_col(df, "NSC_CODE")
            df = ensure_date_column(df, "date")
            keep = [
        ...
        logger.info(
            f"[MSG合并完成] 总行数: {df.height}, 总列数: {len(df.columns)}"
        )
        ...
        if df.height > 0:
            ...
            for col in numeric_cols:
                if col in df.columns:
                    ...
                    logger.info(
                        f"[MSG统计] {col}: 总和={total}, 非空值={non_null}, 空值={(df.height - non_null)}, 类型={df[col].dtype}"
                    )
        group_cols = ["NSC_CODE", "date"]
        sum_cols = [
            "enter_private_count",
        ...
        if agg_exprs:
            ...
            df = df.unique(subset=group_cols)
        logger.info(
            f"[MSG聚合完成] 聚合后行数: {df.height}, 聚合后列数: {len(df.columns)}"
        )
        ...
        if "NSC_CODE" not in df.columns:
            ...
            raise ValueError(
                f"[MSG聚合后] 没有 NSC_CODE，实际列: {df.columns}"
            )
        dfs["msg"] = df
    ...
    if "account_bi_file" in local_paths:
        ...
        if df is not None:
            ...
            dfs["account_bi"] = process_single_table(
                df,
                ACCOUNT_BI_MAP,
                ["live_leads", "short_video_plays"],
                "account_bi",
            )
            logger.debug(
                f"[ACC_BI PROBE] Processed account_bi data:\n{dfs['account_bi'].head()}"
            )
    # 5. leads_file
    if "leads_file" in local_paths:
        ...
        if df is not None:
            ...
            dfs["leads"] = process_single_table(
                df, LEADS_MAP, ["small_wheel_leads"], "leads"
            )
    # 6. DR1_file, DR2_file
    dr_frames = []
    ...
    if "Spending_file" in local_paths:
        ...
        if ".csv" in p.lower() or ".txt" in p.lower():
            ...
            if spending_sheet_names:
                ...
                sheet_names = [
                    s.strip()
                    for s in normalized_sheet_names.split(",")
                    if s.strip()
                ]
            if sheet_names:
                processed_dfs = []
                for s in sheet_names:
                    try:
                        ...
                        processed = process_single_table(
                            df_sheet,
                            SPENDING_MAP,
                            ["spending_net"],
                            "spending",
                        ...
                        logger.warning(
                            f"Spending_file sheet {s} processing failed: {e}"
                        )
                        continue
                if processed_dfs:
                    ...
                    df = process_single_table(
                        df_sheet, SPENDING_MAP, ["spending_net"], "spending"
                    )
            else:
                ...
                for sheet_name, df_sheet in all_sheets.items():
                    try:
                        ...
                        logger.warning(
                            f"Spending_file sheet {sheetname} processing failed: {e}"
                        )
                        continue
                if processed_dfs:
                    df = pl.concat(processed_dfs, how="vertical")
        ...
        if df is not None:
            ...
            dfs["spending"] = process_single_table(
                df, SPENDING_MAP, ["spending_net"]
            )
    # 8. account_base_file
    account_base = None
    if "account_base_file" in local_paths:
        ...
        if account_base is not None:
            logger.info(
                f"Account base processed successfully: {account_base.shape[0]} records"
            )
        else:
            logger.warning("Account base processing returned None")
    if not dfs:
        raise ValueError(
            "No dataframes were processed. Check file inputs and names."
        )
    logger.info(
        "Building base keys using pure Python to bypass Polars concat bug."
    )
        ...
        try:
            ...
            if "NSC_CODE" in v.columns and "date" in v.columns:
                ...
                try:
                    ...
                    logger.debug(
                        f"Extracted {len(keys_from_df)} valid keys from {k}"
                    )
                    ...
                    # 如果没有过滤成功，至少添加原始数据
                    keys_from_df = (
                        v.select(["NSC_CODE", "date"]).drop_nulls().to_dicts()
                    )
                    for row in keys_from_df:
                        unique_keys_set.add((row["NSC_CODE"], row["date"]))
            else:
                logger.warning(
                    f"Skipping {k}: missing NSC_CODE or date columns"
                )
        except Exception as e:
            logger.error(f"Error processing dataframe {k}: {str(e)}")
            logger.error(
                f"DataFrame info: columns={list(v.columns) if hasattr(v, 'columns') else 'unknown'}, type={type(v)}"
            )

    if not unique_keys_set:
        raise ValueError(
            "No valid (NSC_CODE, date) key pairs found in any file."
        )
    ...
    for name, df in dfs.items():
        ...
        for col in numeric_cols:
            if col in df.columns:
                ...
                logger.info(
                    f"[数据验证] {name}.{col}: 总和={total}, 空值={null_count}"
                )

    # 无论维度是什么，都必须先创建完整的 base DataFrame 并合并所有数据
    logger.info("统一创建包含所有数据的 base DataFrame")
    base = pl.DataFrame(
    ...
    unique_months = (
        base.get_column("date").dt.month().unique().sort(descending=True)
    )
    ...
    base = base.with_columns(
        period=pl.when(pl.col("date").dt.month() == month_t)
        .then(pl.lit("T"))
        .when(pl.col("date").dt.month() == month_t_minus_1)
        .then(pl.lit("T-1"))
        .otherwise(pl.lit(None))
    ).filter(pl.col("period").is_not_null())
    t_days = (
        base.filter(pl.col("period") == "T")
        .select(pl.col("date").n_unique())
        .item()
        or 1
    )
    t_minus_1_days = (
        base.filter(pl.col("period") == "T-1")
        .select(pl.col("date").n_unique())
        .item()
        or 1
    )
    ...
    logger.info(
        f"维度清理结果: 原始='{original_dimension}' -> 处理后='{dimension}'"
    )

    logger.info(f"使用聚合维度: '{dimension}' (原始参数值)")
    logger.info(f"dimension参数类型: {type(dimension)}")
    logger.info(
        f"维度判断结果: dimension == 'level' -> {dimension == 'level'}"
    )
    logger.info(
        f"维度判断结果: dimension == 'NSC_CODE' -> {dimension == 'NSC_CODE'}"
    )
    ...
    logger.info(
        f"base列: {list(base.columns)[:5] if hasattr(base, 'columns') else '无列信息'}"
    )
    ...
    for col in agg_cols_exist:
        summary_df = summary_df.with_columns(
            (
                pl.col(f"{col}_T").fill_null(0)
                + pl.col(f"{col}_T-1").fill_null(0)
            ).alias(f"{col}_total"),
    ...
    summary_df = summary_df.with_columns(
        ...
        safe_div(
            pl.col("spending_net_total"), (t_days + t_minus_1_days)
        ).alias("avg_daily_spending"),
        safe_div(pl.col("spending_net_t"), t_days).alias(
            "avg_daily_spending_t"
        ),
        safe_div(pl.col("spending_net_t_minus_1"), t_minus_1_days).alias(
            "avg_daily_spending_t_minus_1"
        ),
        safe_div(
            pl.col("paid_area_leads_total"), (t_days + t_minus_1_days)
        ).alias("avg_daily_paid_leads"),
        safe_div(pl.col("paid_area_leads_t"), t_days).alias(
            "avg_daily_paid_leads_t"
        ),
        safe_div(pl.col("paid_area_leads_t_minus_1"), t_minus_1_days).alias(
        ...
        safe_div(
            pl.col("effective_live_hours_25min"), (t_days + t_minus_1_days)
        ).alias("avg_daily_effective_live_hours_25min"),
        safe_div(pl.col("effective_live_hours_25min_t"), t_days).alias(
            "avg_daily_effective_live_hours_25min_t"
        ),
        safe_div(
            pl.col("effective_live_hours_25min_t_minus_1"), t_minus_1_days
        ...
        safe_div(
            pl.col("enter_private_count_total"), (t_days + t_minus_1_days)
        ).alias("avg_daily_private_entry_count"),
        safe_div(pl.col("enter_private_count_t"), t_days).alias(
            "avg_daily_private_entry_count_t"
        ),
        safe_div(
            pl.col("enter_private_count_t_minus_1"), t_minus_1_days
        ).alias("avg_daily_private_entry_count_t_minus_1"),
        safe_div(
            pl.col("private_open_count_total"), (t_days + t_minus_1_days)
        ).alias("avg_daily_private_open_count"),
        safe_div(pl.col("private_open_count_t"), t_days).alias(
            "avg_daily_private_open_count_t"
        ),
        safe_div(pl.col("private_open_count_t_minus_1"), t_minus_1_days).alias(
            "avg_daily_private_open_count_t_minus_1"
        ),
        safe_div(
            pl.col("private_leads_count_total"), (t_days + t_minus_1_days)
        ).alias("avg_daily_private_leads_count"),
        safe_div(pl.col("private_leads_count_t"), t_days).alias(
            "avg_daily_private_leads_count_t"
        ),
        safe_div(
            pl.col("private_leads_count_t_minus_1"), t_minus_1_days
    ...
    final_columns.extend(
        [
            "natural_leads_total",
            "natural_leads_t",
            "natural_leads_t_minus_1",
            "ad_leads_total",
            "ad_leads_t",
            "ad_leads_t_minus_1",
            "spending_net_total",
            "spending_net_t",
            "spending_net_t_minus_1",
            "paid_leads_total",
            "paid_leads_t",
            "paid_leads_t_minus_1",
            "area_leads_total",
            "area_leads_t",
            "area_leads_t_minus_1",
            "local_leads_total",
            "local_leads_t",
            "local_leads_t_minus_1",
            "live_effective_hours_total",
            "live_effective_hours_t",
            "live_effective_hours_t_minus_1",
            "effective_live_sessions_total",
            "effective_live_sessions_t",
            "effective_live_sessions_t_minus_1",
            "exposures_total",
            "exposures_t",
            "exposures_t_minus_1",
            "viewers_total",
            "viewers_t",
            "viewers_t_minus_1",
            "small_wheel_clicks_total",
            "small_wheel_clicks_t",
            "small_wheel_clicks_t_minus_1",
            "small_wheel_leads_total",
            "small_wheel_leads_t",
            "small_wheel_leads_t_minus_1",
            "live_leads_total",
            "live_leads_t",
            "live_leads_t_minus_1",
            "anchor_exposure_total",
            "anchor_exposure_t",
            "anchor_exposure_t_minus_1",
            "component_clicks_total",
            "component_clicks_t",
            "component_clicks_t_minus_1",
            "short_video_leads_total",
            "short_video_leads_t",
            "short_video_leads_t_minus_1",
            "short_video_count_total",
            "short_video_count_t",
            "short_video_count_t_minus_1",
            "short_video_plays_total",
            "short_video_plays_t",
            "short_video_plays_t_minus_1",
            "enter_private_count_total",
            "enter_private_count_t",
            "enter_private_count_t_minus_1",
            "private_open_count_total",
            "private_open_count_t",
            "private_open_count_t_minus_1",
            "private_leads_count_total",
            "private_leads_count_t",
            "private_leads_count_t_minus_1",
            "total_cpl",
            "paid_cpl",
            "local_leads_ratio",
            "avg_daily_spending",
            "avg_daily_spending_t",
            "avg_daily_spending_t_minus_1",
            "avg_daily_paid_leads",
            "avg_daily_paid_leads_t",
            "avg_daily_paid_leads_t_minus_1",
            "paid_cpl_t",
            "paid_cpl_t_minus_1",
            "avg_daily_effective_live_hours_25min",
            "avg_daily_effective_live_hours_25min_t",
            "avg_daily_effective_live_hours_25min_t_minus_1",
            "avg_exposures_per_session",
            "avg_exposures_per_session_t",
            "avg_exposures_per_session_t_minus_1",
            "exposure_to_viewer_rate",
            "exposure_to_viewer_rate_t",
            "exposure_to_viewer_rate_t_minus_1",
            "avg_viewers_per_session",
            "avg_viewers_per_session_t",
            "avg_viewers_per_session_t_minus_1",
            "small_wheel_click_rate",
            "small_wheel_click_rate_t",
            "small_wheel_click_rate_t_minus_1",
            "small_wheel_leads_rate",
            "small_wheel_leads_rate_t",
            "small_wheel_leads_rate_t_minus_1",
            "avg_small_wheel_leads_per_session",
            "avg_small_wheel_leads_per_session_t",
            "avg_small_wheel_leads_per_session_t_minus_1",
            "avg_small_wheel_clicks_per_session",
            "avg_small_wheel_clicks_per_session_t",
            "avg_small_wheel_clicks_per_session_t_minus_1",
            "component_click_rate",
            "component_click_rate_t",
            "component_click_rate_t_minus_1",
            "component_leads_rate",
            "component_leads_rate_t",
            "component_leads_rate_t_minus_1",
            "avg_daily_private_entry_count",
            "avg_daily_private_entry_count_t",
            "avg_daily_private_entry_count_t_minus_1",
            "avg_daily_private_open_count",
            "avg_daily_private_open_count_t",
            "avg_daily_private_open_count_t_minus_1",
            "avg_daily_private_leads_count",
            "avg_daily_private_leads_count_t",
            "avg_daily_private_leads_count_t_minus_1",
            "private_open_rate",
            "private_open_rate_t",
            "private_open_rate_t_minus_1",
            "private_leads_rate",
            "private_leads_rate_t",
            "private_leads_rate_t_minus_1",
            "private_conversion_rate",
            "private_conversion_rate_t",
            "private_conversion_rate_t_minus_1",
        ]
    )
